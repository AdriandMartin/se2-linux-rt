...
#           del sistema operativo usando el tracer wakeup
...
OUTPUT_DIR="measurements/scheduling" # directorio en el que volcar los reports
...
echo "Comenzando mediciones de wakeup"
...
for NUM_WORKERS in $(seq ${INI_WORKERS} ${STEP} ${MAX_WORKERS}); do
	...
	for TEST_NUMBER in $(seq ${REPETITIONS}); do
		trace-cmd record -p wakeup stress-ng -q --timeout 10 --${STRESSOR} ${NUM_WORKERS}
...
echo "Finalizadas mediciones de wakeup"
